    ...
    <>
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm transition-colors">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between h-14 sm:h-16 lg:h-18 py-1.5 sm:py-2 gap-1.5 sm:gap-4">
              ...
              {(['SD', 'SMP', 'SMA'] as EducationLevel[]).map((lvl) => {
                ...
                return (
                  <button
                    ...
                    className={`px-2 sm:px-3.5 py-1 sm:py-1.5 rounded-lg text-[11px] sm:text-xs font-bold transition-all min-h-[32px] sm:min-h-[36px] flex items-center justify-center ${
                      isActive
                        ? activeBadge
                        : 'text-slate-600 hover:text-slate-900 hover:bg-white/50'
                    }`}
                  >
                    <span className="hidden md:inline">Jenjang </span>
                    <span>{lvl}</span>
                  </button>
                );
              })}
            ...
            <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
              ...
              {onSwitchUser && (
                <div className="flex items-center bg-slate-100 rounded-xl p-0.5 sm:p-1 border border-slate-200">
                  <span className="text-xs text-slate-500 px-1 hidden xl:inline font-medium">Akun:</span>
                  <select
                    value={currentUser?.id || 'usr-student-elang'}
                    onChange={e => onSwitchUser(e.target.value)}
                    className="bg-white text-slate-800 font-semibold text-[11px] sm:text-xs py-1 sm:py-1.5 px-1.5 sm:px-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-400 cursor-pointer max-w-[100px] sm:max-w-[170px] truncate min-h-[32px] sm:min-h-[36px]"
              ...
              {isSpeaking && (
                <button
                  ...
                  className="flex items-center gap-1 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg bg-rose-50 text-rose-700 border border-rose-200 hover:bg-rose-100 text-[11px] sm:text-xs font-semibold animate-pulse min-h-[32px] sm:min-h-[36px]"
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-200 text-xs font-medium transition-colors min-h-[32px] sm:min-h-[36px]"
                title="Pengaturan Aksesibilitas & Auditori"
              >
                <Sliders className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-sky-600" />
                <span className="hidden lg:inline">Akses</span>
              </button>
            </div>
